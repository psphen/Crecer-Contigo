<div wire:init="readyToLoad">
    <div wire:ignore.self class="card" id="show">
        <div class="card-header">
            <h5 class="card-title mb-3">{{__('Reviews')}}</h5>
        </div>
                ...
                <table class="table">
                    ...
                    @forelse($reviews as $review)
                        <tr>
                            ...
                            <td class="text-center">{{$review->place->name}}</td>
                            <td class="text-center">{{$review->score}}</td>
                            <td class="text-center" style="max-width: 200px"> <p>{{$review->comment}}</p></td>
                            <td class="text-center">
                                @if($review->status==1)
                                    <button type="button" class="btn btn-label-success waves-effect"><i class="ti ti-eye"></i></button>
                                @else
                                    <button type="button" class="btn btn-label-danger waves-effect"><i class="ti ti-eye-off"></i></button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr class="odd">
                            <td valign="top" colspan="12" class="dataTables_empty text-center">{{$readyToLoad?__('No reviews registered'):__('Loading').'...'}}</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
                        ...
                        <div class="row mx-2 my-3">
                            <div class="col-md-5">
                                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                                    {{__('Showing')}} {{ $reviews->firstItem() }} {{__('to')}} {{ $reviews->lastItem() }} {{__('of')}} {{ $reviews->total() }} {{__('results')}}
                                </div>
                            </div>
